<template>
  <div class="mgr-page">
    ...
    <div v-if="showDialog" class="dialog-overlay" @click.self="closeDialog">
      <div class="dialog">
        <h3>{{ editing ? '编辑' : '新增' }}发展历程</h3>
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label>年份</label>
            <input v-model="form.year" class="form-input" required />
          </div>
          <div class="form-group">
            <label>标题</label>
...
import { getTimeline } from '@/api/about'
import { addTimeline, updateTimeline, deleteTimeline } from '@/api/admin'

const list = ref<any[]>([])
const loading = ref(true)
const showDialog = ref(false)
const editing = ref(false)
const form = ref<any>({ year: '', title: '', description: '', sortOrder: 0 })
...
function openAdd() {
  editing.value = false
  form.value = { year: '', title: '', description: '', sortOrder: 0 }
...
async function handleSubmit() {
  try {
    if (editing.value) {
      await updateTimeline(form.value.id, form.value)
    } else {
      await addTimeline(form.value)
    }
    closeDialog()
    await loadData()
  } catch (e) {
...
.mgr-table th {
  background: var(--bg-light); padding: 12px 16px; text-align: left;
  font-size: 13px; font-weight: 600; color: var(--text-secondary); border-bottom: 1px solid var(--border);
}
.mgr-table td {
