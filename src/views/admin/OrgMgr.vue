...
import { getOrg } from '@/api/about'
import { addOrg, updateOrg, deleteOrg } from '@/api/admin'

const list = ref<any[]>([])
const loading = ref(true)
const showDialog = ref(false)
const editing = ref(false)
const form = ref<any>({ name: '', description: '', sortOrder: 0 })
...
async function handleSubmit() {
  try {
    if (editing.value) await updateOrg(form.value.id, form.value)
    else await addOrg(form.value)
    closeDialog()
    await loadData()
...
async function handleDelete(id: number) {
  ...
  try { await deleteOrg(id); await loadData() }
