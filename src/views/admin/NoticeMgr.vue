<template>
  <div class="mgr-page">
    ...
    <table class="mgr-table" v-if="!loading">
      ...
      <tbody>
        <tr v-for="item in list" :key="item.id">
          <td>{{ item.tag }}</td>
          <td class="desc-cell">{{ item.title }}</td>
          <td>{{ item.date }}</td>
    ...
    <div v-if="showDialog" class="dialog-overlay" @click.self="closeDialog">
      <div class="dialog">
        <h3>{{ editing ? '编辑' : '新增' }}公告</h3>
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label>标签</label>
            <select v-model="form.tag" class="form-input" required>
...
import { addNotice, updateNotice, deleteNotice } from '@/api/admin'

const list = ref<any[]>([])
const loading = ref(true)
const showDialog = ref(false)
const editing = ref(false)
const form = ref<any>({ tag: '公司动态', title: '', date: '', department: '', summary: '', sortOrder: 0 })
...
function openAdd() {
  editing.value = false
  form.value = { tag: '公司动态', title: '', date: '', department: '', summary: '', sortOrder: 0 }
...
async function handleSubmit() {
  try {
    if (editing.value) await updateNotice(form.value.id, form.value)
...
async function handleDelete(id: number) {
  ...
  try { await deleteNotice(id); await loadData() }
